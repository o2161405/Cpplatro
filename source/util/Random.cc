...
f64 Random::pseudoSeed(const std::string &key, const std::optional<std::string> &predictSeed) {
    ...
    constexpr f64 FLOAT_TRUNCATION = 1e13;

    if (predictSeed) {
        f64 predictHash = pseudoHash(key + *predictSeed);
        predictHash = std::fmod(HASH_BASE + predictHash * HASH_MULTI, 1.0);
        predictHash = std::round(std::abs(predictHash) * FLOAT_TRUNCATION) / FLOAT_TRUNCATION;
        ...
        cachedState = std::round(std::abs(cachedState) * FLOAT_TRUNCATION) / FLOAT_TRUNCATION;
